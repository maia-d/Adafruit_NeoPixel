...
boolean branch(int n) {
  ...
  for(int x = 0; x<branchies.length();
  if branchies[n]
    return true;
  }
  return false;
...
void colorWipe() {
  for(int i=0; i<strip.numPixels(); i++) {
    ...
    if (branch(i)== false){
      ...
      strip.setPixelColor(i, strip.Color(150, 75, 0));
    }
